defmodule Aoc2020.Days.D_14 do
  use Aoc2020.Days.Base
  use Bitwise

  def parse_input_str(str), do: Util.str_array(str) |> Enum.map(&parse_line/1)
  ...
  def part_one(str) do
    ...
    {"Part 1:", state[:mem] |> Map.values() |> Enum.sum()}
  end

  @impl true
  def part_two(str) do
    state = str |> parse_input_str() |> run_all_instructions(:part2)
    {"Part 2:", state[:mem] |> Map.values() |> Enum.sum()}
  end
