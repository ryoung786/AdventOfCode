...
class Formula
    attr_accessor :reactants, :product
    def initialize(input_str)
        pairs = input_str.scan(/\d+ \w+/)
        components = pairs.map{|s| x=s.split(' '); Component.new(x[1], x[0].to_i)}
        @product = components.pop
        @reactants = components
    end

    def inspect
        @reactants.map(&:inspect).join(' + ') + ' => ' + @product.inspect
    end
end
